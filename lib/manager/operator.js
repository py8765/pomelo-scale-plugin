		...
		if(!!servers && !!servers.length) {
			(function(key) {
				async.map(servers, function(server, callback) {
					runners[self.type](server.pid, function(err, data) {
						callback(err, data);
					});
				}, function(err, results) {
					if(!!err)	{
						console.error('check cpu with error.');
						return;
					}
...
pro.scale = function(type, results) {
	console.error('key: %j, value: %j', type, results);
	var total = 0;
	for(var i=0; i<results.length; i++)	{
		total += Number(results[i]);
	}
	console.error('total value: %s', total);
	var average = Math.round(total/results.length);
	console.error('average value: %s', average);
	if(average > this.condition[type]) {
		console.error('need to scale.');
		...
		console.error('do not need to scale.');
	}
};
